...
class SOM():
    ...
    def initialize_neuron_weights(self) -> None:
        ...
        """
        # Reset the weights
        if self.weights == []: self.reset_weights()
    ...
    def train_visual(self):
        ...
        ''' 
        values = self.data.values()
        self.radius = self.initial_radius * (1 - (self.visual_iteration / self.visual_iterations))
        print(self.radius)
        for x in values:
            # Find the Best Matching Unit (BMU)
            bmu_idx = self.find_best_matching_unit(x)
            # Update the weights of the SOM
            self.update_weights(x, bmu_idx, self.visual_iteration, self.visual_iterations)
        self.visual_iteration += 1
